...
И в последний момент попросилось обозначиться как отдельный боундэд-контекст "управление заказами" в составе этого же поддомена. ES уже не стал перелопачивать, так как там всё тесно переплетается.
